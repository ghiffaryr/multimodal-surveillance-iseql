...
\begin{itemize}
  ...
  \item \textbf{Condition B (audio-only).} PANNs CNN14 or Qwen2-Audio-7B-Instruct extracts environmental audio events, persists them as temporal intervals, and the event detector runs four audio-only SQL queries.
  \item \textbf{Condition C (multimodal).} Both pipelines run. The event detector runs six multimodal SQL queries that take the union of visual and audio results. Events with no acoustic correlate (handoff, suspicious near vehicle) reuse the visual queries from condition A.
\end{itemize}
...
\begin{table}[h]
  ...
  \begin{tabular}{l p{3cm} p{3.8cm}}
    ...
    B (audio)   & PANNs / Qwen2-Audio-7B & 4 audio-only SQL \\
