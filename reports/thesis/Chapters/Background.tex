...
The eight ISEQL operators are parametrised by distance thresholds $\delta$ (start offset) and $\varepsilon$ (end offset). Table~\ref{tab:iseql_ops} defines the operators used in the event models of this thesis, each with an inline interval diagram and its formal constraint; $\delta$ and $\varepsilon$ are frame-count tolerances. This thesis uses primarily \textbf{BEFORE} (Bef) and \textbf{SP} (Start Preceding). When thresholds are omitted, maximum allowed values are used.

\begin{table}[h]
  \centering
  \caption{ISEQL operators used for event detection.}
  \label{tab:iseql_ops}
  \begin{tabular}{@{}p{9.5em}p{6.2em}p{11em}@{}}
    \toprule
    Operator & Diagram & Definition \\
    \midrule
    \textsc{Before} ($\Bef_\delta$) &
    \DoodleBef &
    $0 < s.T_s - r.T_e \le \delta$ \\
    \addlinespace
    \textsc{Start Preceding} ($\SP_\delta$) &
    \DoodleSP &
    $0 \le s.T_s - r.T_s \le \delta$\newline
    $r.T_e - s.T_s \ge 0$ \\
    \addlinespace
    \textsc{Left Overlap Join} ($\LOJ_{\delta,\varepsilon}$) &
    \DoodleLOJ &
    $0 \le s.T_s - r.T_s \le \delta$\newline
    $0 \le s.T_e - r.T_e \le \varepsilon$\newline
    $0 \le \delta < \mathrm{size}(r)$\newline
    $0 \le \varepsilon < \mathrm{size}(s)$ \\
    \addlinespace
    \textsc{During Join} ($\DJoin_{\delta,\varepsilon}$) &
    \DoodleDJ &
    $0 \le r.T_s - s.T_s \le \delta$\newline
    $0 \le s.T_e - r.T_e \le \varepsilon$\newline
    $0 \le \delta \le \mathrm{size}(r) + \varepsilon$\newline
    $0 \le \varepsilon \le \mathrm{size}(r) + \delta$ \\
    \addlinespace
    \textsc{End Following} ($\EF_\varepsilon$) &
    \DoodleEF &
    $0 \le r.T_e - s.T_e \le \varepsilon$\newline
    $s.T_e - r.T_s \ge 0$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
The subscripts follow the three-condition design used throughout this thesis: condition \textbf{A} (visual-only) evaluates the visual interval labeling, condition \textbf{B} (audio-only) evaluates the audio interval labeling, and condition \textbf{C} (multimodal) is the set union of A and B. Events without an acoustic correlate (handoff, loitering) are defined only under condition A and are unchanged in C. The concrete query definitions for the six event types evaluated in this thesis are given in Section~\ref{sec:event-definitions}.
