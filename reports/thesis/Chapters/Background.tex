...
\begin{itemize}
  \item \textbf{Before} ($R \ \mathrm{Bef}_{\delta}\ S$): Returns pairs where $r$ ends before $s$ starts, with a gap of at most $\delta$ frames. Condition: $0 < s.T_s - r.T_e \le \delta$. Corresponds to Allen's \textit{before}.
...
Below, $M_i.\mathrm{arg}_j$ denotes the $j$-th argument of interval $M_i$, and $M_i.s_f$, $M_i.e_f$ denote its start and end frames.
