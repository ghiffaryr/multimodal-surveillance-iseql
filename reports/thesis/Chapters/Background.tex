...
\]
includes $n$ explicit attributes $A_i$ and two temporal attributes $T_s, T_e \in \mathbb{N}$ denoting the interval endpoints. The size of an interval is $\mathrm{size}(r) = r.T_e - r.T_s + 1$. An interval labeling $M$ is an interval relation whose attributes correspond to predicate arguments. Throughout this thesis, $M_i.\mathrm{arg}_j$ denotes the $j$-th argument of interval $M_i$, and $M_i.s_f$, $M_i.e_f$ denote its start and end frames.
