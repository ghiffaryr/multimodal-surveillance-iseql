...
\node[font=\Large\bfseries] at (2.5,6.9) {Audio Pipeline -- Condition B};

\node[box,fill=cBlue] (s1) at (2.5,4.8) {
    \textbf{Audio Extraction}\\[2pt]
    FFmpeg, 16 kHz mono PCM
};

\node[box,fill=cGreen] (s2) at (2.5,2.8) {
    \textbf{Model Inference}\\[2pt]
    Qwen2-Audio / PANNs, 2.5s window
};

\node[box,fill=cBlue] (s3) at (2.5,0.8) {
    \textbf{Label to Event}\\[2pt]
    AudioSet labels $\to$ 4 sound queries
};

\node[box,fill=cGreen] (s4) at (2.5,-1.2) {
