...
\begin{tikzpicture}[
  ...
  box/.style={draw=black!50, rounded corners=3pt, align=center, inner sep=5pt, minimum height=1.6cm},
...
\node[box,fill=cRed, minimum width=13cm] (L4) at (7,8.5) {
...
\node[box,fill=cYellow, minimum width=13cm] (L3) at (7,6.0) {
    \textbf{Storage Layer}\\[3pt]
    SQLite: VisualPerFrame, VisualRelation, VisualPerInterval, VisualParticipant, SoundPerInterval
};

\node[box,fill=cGreen, minimum width=13cm] (L2) at (7,3.5) {
    \textbf{Reasoning Layer}\\[3pt]
    ISEQL+ SQL: Handoff, Loitering, Vehicle Escape, Fight,\\
    Vehicle Collision, Gunshot / Explosion, Condition A / B / C Logic
};

\node[box,fill=cBlue, minimum width=13cm] (L1) at (7,1.0) {
