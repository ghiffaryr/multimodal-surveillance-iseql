...
- **Audio pipeline**: PANNs CNN14 or Qwen2-Audio-7B-Instruct → AudioPerInterval
- **Visual pipeline**: VLM (Pixtral 12B, Ministral 3-14B, Gemini 2.5 Flash, Gemini 3.6 Flash) → VisualRelation → VisualPerInterval
- **High-level detection**: ISEQL temporal queries over VisualPerInterval and AudioPerInterval
...
| B (sound) | Event has audio correlate | Handoff, suspicious_near_vehicle (no sound) excluded |
...
- B: `fight` = shout BEFORE impact WITHIN delta_sound_fight on AudioPerInterval.
...
- B: `fight` = shout BEFORE impact on AudioPerInterval → TP.
...
- B: `gunshot` = gunshot + `explosion` = explosion on AudioPerInterval.
...
- B: `gunshot` = gunshot on AudioPerInterval → TP.
...
- B: `explosion` = explosion on AudioPerInterval → TP.
...
- B: `vehicle_escape` = engine SP tire_squeal on AudioPerInterval.
...
- B: `vehicle_collision` = (car_horn) SP (impact / glass_breaking) on AudioPerInterval → TP.
...
- B: `vehicle_collision` = (skidding) SP (impact / glass_breaking) on AudioPerInterval → TP.
- C: Both → TP.
...
## Scene 18: Suspicious Near Vehicle (Peering Into Car)
...
- A: `suspicious_near_vehicle` = relation "suspicious_near_vehicle" WITH duration (M1.et − M1.st) ≥ 3s [visual only].
- C: `suspicious_near_vehicle` = relation "suspicious_near_vehicle" WITH duration (M1.et − M1.st) ≥ 3s [visual only].

| Cond | Expected | Rationale |
| ---- | -------- | --------- |
| A    | TP       | VLM detects peering into car; interval spans 3.0s |
...
| Modality | Class | Start (s) | End (s) | Start Frame | End Frame | Description |
|----------|-------|-----------|---------|-------------|-----------|-------------|
| visual   | suspicious_near_vehicle   |    2.000 |    5.000 |    48 | 120   | Person peers into car window |

---


## Scene 19: Suspicious Near Vehicle (Crouching Near SUV)
...
- A: `suspicious_near_vehicle` = relation "suspicious_near_vehicle" WITH duration (M1.et − M1.st) ≥ 3s. The person stays crouched near the car for ~6 seconds → TP.
- C: `suspicious_near_vehicle` = relation "suspicious_near_vehicle" WITH duration (M1.et − M1.st) ≥ 3s (same as visual) → TP.

| Cond | Expected | Rationale |
| ---- | -------- | --------- |
| A    | TP       | Person crouches near SUV >= 72 frames / 3s |
| C    | TP       | Visual-only query preserved |
...
## Scene 20: Suspicious Near Vehicle (Delivery Van)
...
- A: `suspicious_near_vehicle` = relation "suspicious_near_vehicle" WITH duration (M1.et − M1.st) ≥ 3s. Person inspects van for ~5 seconds → TP.
- C: `suspicious_near_vehicle` = relation "suspicious_near_vehicle" WITH duration (M1.et − M1.st) ≥ 3s (same as visual) → TP.

| Cond | Expected | Rationale |
| ---- | -------- | --------- |
| A    | TP       | Person inspects delivery van >= 72 frames / 3s |
| C    | TP       | Visual-only query preserved |
...
- A: `fight` = physical_altercation on VisualPerInterval.
- B: `fight` = shout BEFORE impact on AudioPerInterval.
- C: `fight` = (shout BEFORE impact) UNION physical_altercation.
...
- B: `vehicle_collision` = (car_horn) SP (impact / glass_breaking) on AudioPerInterval.
...
- A: `vehicle_collision` = vehicle_collision on VisualPerInterval.
- B: `vehicle_collision` = (skidding) SP (impact / glass_breaking) on AudioPerInterval.
...
## Scene 35: Suspicious Near Vehicle: Person Inspecting Parked Car
...
| Cond | Expected | Rationale |
| ---- | -------- | --------- |
| A    | TP       | Suspicious_near_vehicle interval spans 3.0s |
| C    | TP       | Visual-only query preserved |
...
## Scene 36: Suspicious Near Vehicle: Person Inspecting Parked SUV
