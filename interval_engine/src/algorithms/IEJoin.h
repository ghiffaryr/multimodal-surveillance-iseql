...
struct OffsetIntervalEnd
{
	Timestamp offset;


	OffsetIntervalEnd(Timestamp offset)
	:
		offset(offset)
	{
	}
...
public:
	IEJoinOperator2(GetRA1&& getRA1, GetSA1&& getSA1, GetRA2&& getRA2, GetSA2&& getSA2,
...
private:
	...
	static void relationsToElements(GetRValue&& getRValue, GetSValue&& getSValue, const Relation& R, const Relation& S, std::vector<Element>& result)
	{
...
using IEJoinBefore = IEJoinOperator2<
	OffsetIntervalEnd, Operator::Less,    GetIntervalStart,
	OffsetIntervalEnd, Operator::Greater, GetIntervalStart>;



inline auto createIEJoinStartPrecedingStrict(const Relation& R, const Relation& S)
{
	return IEJoinStartPrecedingStrict(GetIntervalStart(), GetIntervalStart(), GetIntervalEnd(), GetIntervalStart(),
		R, S);
}


inline auto createIEJoinReverseDuringStrict(const Relation& R, const Relation& S)
{
	return IEJoinReverseDuringStrict(GetIntervalStart(), GetIntervalStart(), GetIntervalEnd(), GetIntervalEnd(),
		R, S);
}


inline auto createIEJoinBefore(const Relation& R, const Relation& S, Timestamp delta)
{
	return IEJoinOperator2<
		OffsetIntervalEnd, Operator::Less,    GetIntervalStart,
		OffsetIntervalEnd, Operator::Greater, GetIntervalStart
	>{
		OffsetIntervalEnd(-1),        GetIntervalStart(),
		OffsetIntervalEnd(delta + 1), GetIntervalStart(),
	R, S};
}
...
{
	createIEJoinStartPrecedingStrict(R, S).join(consumer);
}
...
{
	createIEJoinReverseDuringStrict(R, S).join(consumer);
}



template <typename Consumer>
void ieJoinBeforeJoin(const Relation& R, const Relation& S, Timestamp delta, Consumer&& consumer) noexcept
{
	createIEJoinBefore(R, S, delta).join(consumer);
}
